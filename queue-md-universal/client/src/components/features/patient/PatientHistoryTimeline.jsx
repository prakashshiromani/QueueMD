  ...
  if (!visits || visits.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 px-4 text-center font-inter animate-in fade-in duration-300">
        <span className="material-symbols-outlined text-5xl text-text-secondary mb-3 opacity-60">folder_open</span>
        <h3 className="text-sm font-bold text-text-primary mb-1">No Past Records</h3>
        <p className="text-xs text-text-secondary max-w-[200px] leading-relaxed">No past clinical records or prescriptions found for this patient.</p>
      </div>
    );
  }
    ...
    <>
      ...
      >
        {visits.map((visit, index) => (
          ...
          >
            {/* Timeline Dot */}
            <div className="absolute -left-[9px] top-1 w-4 h-4 rounded-full bg-blue-600 ring-4 ring-bg-secondary dark:ring-[#0B1120]"></div>
            
            {/* Glassmorphism Card (design.md rule) */}
            <div className="bg-bg-primary dark:bg-white/5 backdrop-blur-md border border-border-muted dark:border-white/10 rounded-xl p-4 shadow-lg hover:bg-bg-primary dark:hover:bg-white/10 transition-all">
              ...
              <div className="flex justify-between items-start mb-2">
                ...
                <div className="flex gap-2">
                  <button 
                    ...
                    className="text-xs px-3 py-1 bg-green-100 dark:bg-green-600/20 text-green-700 dark:text-green-400 rounded-full hover:bg-green-200 dark:hover:bg-green-600/40 transition flex items-center gap-1 font-medium"
                  >
                  ...
                  <motion.div 
                      ...
                      className={`border-l-4 pl-4 py-3 mt-3 rounded-r-lg ${
                          doc.uploadedBy === 'patient' 
                              ? 'border-yellow-500 bg-yellow-50 dark:bg-yellow-500/10'  // Patient uploaded = Yellow
                              : 'border-blue-500 bg-blue-50 dark:bg-blue-500/10'      // Doctor uploaded = Blue
                      }`}
                  >
                      ...
                      <a href={doc.url} target="_blank" rel="noreferrer" className="text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 font-medium underline text-sm transition-colors inline-flex items-center gap-1">
                          <span className="material-symbols-outlined text-[16px]">visibility</span> View {doc.type?.includes('pdf') ? 'PDF Document' : 'Image'}
                      </a>
                      
                      {doc.uploadedBy === 'patient' && (
                          <p className="text-[11px] text-yellow-700 dark:text-yellow-400/80 mt-2 italic flex items-center gap-1">
