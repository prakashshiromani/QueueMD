...
import { Upload, Lock, FileText, LogOut, Calendar, Activity, RefreshCcw } from 'lucide-react';
                                                    ...
                                                    historyData.flatMap((visit) => 
                                                        (visit.documents || []).map((doc) => (
                                                            <button
                                                                key={doc._id}
                                                                onClick={(e) => handleViewAndDownload(e, doc.url, doc.fileName)}
                                                                className="p-3 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-blue-500/30 rounded-xl transition-all block overflow-hidden text-left w-full cursor-pointer group"
                                                            >
                                                                <div className="flex items-center gap-2 mb-1.5 overflow-hidden">
                                                                    <FileText className="w-4 h-4 text-blue-400 flex-shrink-0 group-hover:text-blue-400 transition-colors" />
                                                                    <span className="text-[11px] font-bold text-gray-200 truncate group-hover:text-blue-400 transition-colors">{doc.fileName}</span>
                                                                </div>
                                                                <div className="flex justify-between items-center text-[9px] text-gray-500 uppercase tracking-wide">
