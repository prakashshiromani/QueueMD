 ...
 * @description Cloudinary Media Service Layer
 * 
 * Handles all Cloudinary asset management operations.
 * Wraps cloudinaryHelper utilities for a clean service interface.
 */
...
/**
 * Delete an asset from Cloudinary by its public URL or ID.
 * 
 * @param {string} publicIdOrUrl - Cloudinary public_id or the full public URL of the asset
 * @param {string} resourceType - 'image' | 'raw' | 'video'
 * @returns {Promise<Object>} Cloudinary deletion result
 */
exports.deleteAsset = async (publicIdOrUrl, resourceType = 'image') => {
  try {
    ...
    if (!publicId) {
      throw new Error("Could not parse Cloudinary publicId");
    }

    const result = await cloudinary.uploader.destroy(publicId, { resource_type: resourceType });
    logger.info(`[CLOUDINARY SERVICE] Deleted asset: ${publicId}`);
    return result;
  } catch (err) {
    logger.error(`[CLOUDINARY SERVICE] Delete failed for ${publicIdOrUrl}: ${err.message}`);
    throw err;
  }
};
